...
{
   //element is the item being searched for
   int iBegin = 0;
   int iEnd = current_size - 1;
   //if the iEnd equals 0 or the beginning is less than the iEnd
...
{
   /* If iterator returned is the same as the end, but the item is
            not the last, this means that the item was not found, so we
            can't delete it - it does not exist */
